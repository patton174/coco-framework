...
- 一条受管汇总评论展示全部角色、执行状态、共识、争议和上下文来源。
...
- `evidence-verifier`：逐条核对 P0/P1 finding 的文件、行号、代码 anchor、触发场景和
...
两个 verifier 始终各自执行一次独立模型调用。没有 P0/P1 候选时也必须返回空验证数组，
不能由协调器伪造一个“未需要验证”的模型结果。
...
- 任一验证者 `UNVERIFIED`：进入 unverified，不阻断，但在评论中保留。
- P2/P3 永不直接阻断。
...
这与项目已有审计方法一致：非低危 finding 需要独立验证，默认未验证为 false，避免把
单 Agent 的合理措辞误当成事实。
...
才变为 success。旧 commit 的 approval 不计入。

## 评论和可见性

使用一个 marker：`<!-- agent-jury:v1 -->`。单条评论是评审团汇总面板，不代表单 Agent。
...
- 单元测试覆盖 context 预算、SHA 绑定、role schema、cross-review 三态和确定性 verdict。
...
- PR 更新后旧 run 不能向新 head 发布评论或 success status；同一 head 的旧 run 也不能
  覆盖带有更高 run ID/run attempt 的受管评论。
