...
| D31 | web | correctness | `CocoWebExceptionHandler.handleUnhandledException` 抓所有 5xx，吞掉客户端断开信号 |
| D32 | web | security | 过滤器顺序：加密先于重放，攻击者可消耗 CPU 后才被拒（与 B7 重叠） |
...
| D35 | web | correctness | `CocoFilterExceptionResponseWriter` 用 `RequestContextHolder` 侧面拿数据，并行错误渲染下不线程安全（与 B9 重叠） |
| D36 | web | correctness | `BodyCache` filter 触发签名 / 加密的检测用手写 `split("&")`（与 B10 重叠） |
| D37 | web | correctness | `CocoTraceFilter.restoreMdcValue` 用 `==` 判空（与 B11 重叠） |
