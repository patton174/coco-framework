 ...
 * Coco 异步日志溢出可观测性自动配置。
 * <p>
 * 该配置必须早于标准日志自动配置，使没有业务自定义 listener 时只创建一个组合 listener。业务侧
 * {@code CocoAsyncLogDropListener} Bean 会在自动配置处理前被发现并保持完全替换语义。
 * </p>
 */
...
public class CocoObservabilityLoggingAutoConfiguration {
    ...
    public io.github.coco.logging.core.CocoAsyncLogDropListener cocoObservabilityAsyncLogDropListener(
        ...
        return new CocoObservabilityAsyncLogDropListener(observationProvider);
    }
}
