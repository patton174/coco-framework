...
Canonical integration owns those executable claims: the tracked current facade POMs and `coco-maven-plugin` archive fixtures are built by the reactor Maven install in `reusable-tests.yml`, while `CocoPackagePruneMojoTest` verifies Boot archive and index pruning. `verify_boot_archive_feature_coordinates.py` retains generic archive-inspection unit coverage. A physical compatibility-module move must update that canonical Maven/Python integration in the same implementation PR; it must not be represented as an implemented module solely by a governance routing fixture.
