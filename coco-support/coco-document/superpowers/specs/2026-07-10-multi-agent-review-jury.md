...
- 可解析 JSON 先校验 `schema_version`、受保护角色、`head_sha`、`context_sha256`；仅整数 `1`；身份/binding不符立即失败关闭。其余字段/类型/数组/枚举/范围/引用/权限错误可在同一受保护 prompt/角色/binding下纠错；specialist/chair可含不可信原task/上次输出/错误。其余纠错仅传原task/上次SHA-256/不回显值的精确错误，完整替代，禁嵌入上次输出/清洗非法evidence。cross-review protected system由已验证`context_evidence_sources(context)`生成稳定排序catalog，仅列`trust_domain`、`path`、可用行区间，不列source内容；目录外/越界仍拒绝，政策/代码域授权立即失败关闭。每Agent最多三次，第三次无效时基础设施BLOCK；拒答、API/鉴权/传输、非法envelope、角色/SHA/hash/binding不匹配立即失败关闭。
