...
- 不把 fork、bot 或 PR head 代码放入拥有仓库密钥的执行路径。
...
chair、fork/非受信 bot 的 no-secret reviewer，也不进入任何 PR-head 代码。同仓库 PR 只有作者
是普通用户，或作者 login、Bot 类型和不可变 Bot ID 与仓库固定的 Coco Agent App 身份全部一致时，
才进入完整评审团；Dependabot、其他 App 和 fork 不得因名称相似进入密钥路径。
...
任何 Issue API、身份、binding 或 gate 发布失败都失败关闭。fork/非受信 bot 的 no-secret reviewer 不注入 App 私钥、
...
本变更同时修改 `pull_request_target` reviewer 和新增密钥工作流，不能由 PR head 自托管。发布顺序固定：
