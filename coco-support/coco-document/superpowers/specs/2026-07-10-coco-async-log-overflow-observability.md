...
- 保持有界队列的 `offer()` 非阻塞，继续允许队列满时丢弃 `TRACE`、`DEBUG` 和 `INFO`。
...
- `totalDropped` 是当前 `AsyncCocoLogSink` 实例自启动以来的累计实际丢弃序号，从 1 开始且每次回调唯一；并发线程之间不保证回调开始或完成顺序。
- 回调在提交日志的调用线程中执行，业务实现必须快速且不得主动阻塞；框架只保证队列提交不等待，并隔离监听器运行时异常。
- Sink 使用线程级重入保护。监听器再次调用 Coco 日志并触发溢出时，嵌套记录仍计入累计数，但跳过嵌套通知，避免递归或栈溢出。
- 默认异步输出链路启用时，业务定义唯一 `CocoAsyncLogDropListener` Bean 会使默认 SLF4J 监听器回退；没有自定义 Bean 时自动使用默认监听器。
- 默认异步输出链路启用且存在多个监听器 Bean 时遵循 Spring 单值注入语义：业务必须使用 `@Primary` 或收敛为一个组合监听器，否则启动失败，不静默选择。异步输出关闭或自定义 `CocoLogSink` 接管时不解析该监听器。
...
该策略提供随压力增长的持续信号，同时把诊断量限制在对数级。诊断文本属于非本地化运维日志，不进入面向用户的消息 bundle。业务替换监听器后，可以自行选择指标、采样或告警策略。

## 生命周期与并发语义

- 丢弃计数只在 `ArrayBlockingQueue.offer()` 失败且记录没有同步兜底时增加。
- 队列满时同步写出的 `WARN` 不计入 dropped；始终同步写出的 `ERROR` 或携带异常记录也不计入 dropped。
- `close()` 后提交的记录继续沿用现有同步写出行为，不产生虚假丢弃计数或通知；与关闭并发的提交也不能滞留在已停止消费的队列中。
- 计数使用并发安全原语，多个调用线程同时溢出时不得丢失增量；监听器收到的累计序号必须唯一，但跨线程调用顺序不作保证。
- 监听器抛出的运行时异常被隔离，不能中断 `log()`，也不能停止后台 writer。
- 同一线程中的监听器重入只跳过嵌套通知，不跳过嵌套记录的丢弃计数。
...
- 队列满时，`TRACE`、`DEBUG` 和 `INFO` 的每条实际拒绝记录各计数一次；非重入丢弃各通知一次。
- 新 Sink 的累计计数初值为 0，旧双参数构造器保持可用并启用默认诊断。
- 回调获得准确的日志级别、句柄名称和唯一累计序号，不获得正文或异常。
- 队列满时，`WARN` 同步写出且不计入 dropped。
- `ERROR` 和携带异常的记录始终同步写出且不进入 drop 通知。
- 并发溢出时最终计数准确，监听累计序号不重复；不要求跨线程回调有序。
- 自定义监听器抛异常时 `log()` 不抛出，后台 writer 仍能继续排空。
- 自定义监听器重入 Coco 日志时不递归通知，嵌套丢弃仍计入累计数。
- 默认监听器只在首次及 2 的幂次累计数输出 WARN，且不经过 Coco 异步日志链路。
- 关闭后写入保持同步回退，不产生 drop 通知。
- 与关闭并发提交的记录最终由后台排空或同步写出，不能永久滞留在队列中。
- 自定义 `CocoAsyncLogDropListener` Bean 替换默认监听器；关闭 async 或自定义 `CocoLogSink` 时不使用溢出链路。
- 默认异步输出启用且多个监听器 Bean 未提供唯一候选时应用明确启动失败，不静默忽略或任选实现；异步输出关闭时不解析监听器候选。
