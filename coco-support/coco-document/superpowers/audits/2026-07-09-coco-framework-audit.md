...
- 在历史 sample 中提供推荐适配器，展示如何把 JWT / 登录主体 → 三个 `ContextHolder`；当前该历史验收已随 samples 退役，由 `coco-admin/framework-acceptance` 维护；
...
**处理状态：** PR6 已增加默认内存 Store 的集群风险 WARN；PR40 进一步提供显式启用的 `JdbcCocoReplayStore` 参考实现，复用业务现有 `JdbcOperations` 和预建表，通过数据库唯一键完成跨实例原子占用。框架不自动建表、不接管数据库迁移或业务事务，也不承诺 exactly-once 副作用。
...
**处理状态：** PR35 已处理。当时的独立 full sample 只通过 `coco-parent`、单 starter 和 H2
...
**建议（L）：** 在业务验收工程中增加 H2 + MP + 启用全部 feature + `CocoSecurity.requireRole` + 一次 `cocoAuditPublisher.publish` + 一次租户绑定查询 + 数据权限过滤。历史验收已随 samples 退役，当前由 `coco-admin/framework-acceptance` 维护。
...
**处理状态：** PR9 先把三个模块的 README 承诺收敛到真实边界；Audit 随后补齐事件发布链路并在 full sample 中验证，OpenAPI 在 PR30 增加 SpringDoc 元数据消费者。PR37 将 Codegen 默认 No-Op 改为真实模板生成器，提供内置显式 CRUD 源码模板、安全文件写入和不绑定默认生命周期的 `coco:generate` goal。PR38 再把 Audit 默认 No-Op 收口为独立结构化日志记录器，同时保留 Formatter / Recorder SPI；数据库、MQ 和合规保留策略继续由业务接管。
...
**处理状态：** PR36 已处理。basic sample 的签名密钥和 AES key 改为
`SAMPLE_SIGNING_KEY` / `SAMPLE_ENCRYPTION_KEY` 环境变量；Python 黑盒脚本未配置时会生成临时材料，
并只通过子 Java 进程环境传递。Postman 生成器不再包含固定 secret、AES key 或 IV，仓库提交的
collection / environment 保持 secret、IV 和密文槽位为空；本地生成默认写入 Git 忽略的
`target/postman`，签名密钥只保存在 Postman environment，不复制到 collection variable，
并由架构测试阻止固定材料回归。重新生成资产时一并将非法密文场景校正为当前框架的 400 响应。
...
**处理状态：** PR22 已通过 Java 可执行文件解析、UTF-8 输出配置和三操作系统 CI 解决 basic
sample 的跨平台执行；PR35 新增的 full sample 黑盒脚本沿用同一进程模型，并在 Ubuntu、Windows、
macOS 矩阵中执行。
...
2. **C9 / C10** 扩展业务验收工程 + README 修正（历史验收已随 samples 退役，当前由 `coco-admin/framework-acceptance` 维护）
