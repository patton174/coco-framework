...
class CocoMessagingStarterDependencyContractTest {

    @Test
    void composesMessagingModule() throws Exception {
        Path starterPom = Path.of(System.getProperty("basedir", ".")).toAbsolutePath().resolve("pom.xml");

        assertThat(directDependencyArtifactIds(starterPom)).contains("coco-messaging");
    }
