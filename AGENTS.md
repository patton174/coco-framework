...
- `coco-feature-codegen` and `coco:generate` are 2.x compatibility: compatibility/security fixes only. `coco-generate` owns new generation; framework must not depend on it.
...
- `coco-feature-archive-smoke` is a non-business Failsafe fixture for packaged feature manifest and `prune-package` indexes.
...
- Release tag creation belongs only to the dedicated Coco Framework Release App. Keep the default workflow token read-only, pin the App Client ID, App ID, slug, installation ID, bot login, and Bot ID in both protected workflow code and the `coco-spring` environment, and grant the App token only `Contents: write` for the single selected repository. GitHub does not expose a tag-only App permission, so the single-repository installation and ruleset boundaries are the compensating least-privilege controls. The `v*` creation ruleset bypasses only this App; branch protection and tag update/deletion have no Release App bypass.
