...
- Agent review may use repository secrets only for same-repository pull requests authored by a human or by the exact configured Coco Agent App login and immutable Bot ID. It runs trusted tooling from the protected base branch, reads PR content as untrusted text through GitHub APIs, and never checks out or executes PR code. Forks and all other bots take a no-secret path and require explicit maintainer approval for the current head SHA.
