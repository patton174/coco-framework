...
Coco 日志默认使用有界异步队列。`ERROR` 和携带异常的记录始终同步写出；队列满时，`WARN` 也会回退为同步输出。被拒绝的 `TRACE`、`DEBUG` 和 `INFO` 仍允许丢弃，避免业务请求线程无限阻塞。

每条实际丢弃都会增加进程内累计计数并通知 `CocoAsyncLogDropListener`。默认监听器直接通过 SLF4J 在首次丢弃以及累计数达到 2 的幂次时输出 WARN，既提供低噪声过载信号，也不会把诊断再次送入已满的 Coco 队列。业务可以用一个 Bean 替换：
...
回调只接收级别、日志句柄名称和累计数，不暴露日志正文或异常；实现必须快速且非阻塞。该机制只提供过载可观测性，不保证日志可靠交付；需要强交付保证的业务应替换 `CocoLogSink` 或审计记录器。
